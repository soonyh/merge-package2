...
function intersect() {
	...
	// if(arguments[0]==="1.9.4-beta"){

	// 	console.log('ranges1',arguments,ranges,pluck(ranges, 'set'))
	// }
	...
	ranges = map(ranges, function(range) {
		...
		console.log('ranges2',lo.operator,hi.operator)
		// if(/1.9.8/.test(lo.semver.raw)){
		...
		// 绝对版本号
		if (lo.operator === '' && hi.operator === '') {
			return lo.operator+lo.semver.raw;
		}
